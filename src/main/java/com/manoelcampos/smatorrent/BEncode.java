...
public class BEncode {
     ...
     */
    protected static byte[] arrayListToByteArray(final List<Byte> al) {
        final byte bytes[] = new byte[al.size()];
     ...
     * @throws InvalidBEncodedStringException When the b-encoded content in the InputStream is malformed
     */
    protected static byte[] getNextField(final InputStream is) throws InvalidBEncodedStringException, IOException {
        int c;
        if (is == null)
            return null;

        final PushbackInputStream pbis = is instanceof PushbackInputStream ? (PushbackInputStream) is : new PushbackInputStream(is);
        c = pbis.read();
        pbis.unread(c);
        ...
        throw new InvalidBEncodedStringException("Unknown header field character '" + (char) c + "'");
    }

    private static byte[] getNextListField(final PushbackInputStream pbis) throws IOException, InvalidBEncodedStringException {
        final var al = new ArrayList<Byte>();
        pbis.read();
        int c;
        while ((c = pbis.read()) != -1)
            al.add((byte) c);
    ...
    private static byte[] getNextDictionaryField(final PushbackInputStream pbis) throws IOException, InvalidBEncodedStringException {
        final var al = new ArrayList<Byte>();
        pbis.read();
        int c;
        while ((c = pbis.read()) != -1)
    ...
    private static byte[] getNextIntegerField(final PushbackInputStream pbis) throws IOException, InvalidBEncodedStringException {
        final var al = new ArrayList<Byte>();
        pbis.read();
        int c;
    ...
    private static byte[] getNextStringField(final PushbackInputStream pbis) throws IOException, InvalidBEncodedStringException {
        int c, i;
        c = pbis.read();
     ...
     *
     * @param is A InputStream containing the b-encoded content
     * @return a map containing the parsed b-encoded fields as byte arrays
     * @throws IOException                    When the file cannot be accessed
     * @throws InvalidBEncodedStringException When the b-encoded content in the InputStream is malformed
     */
    public static final Map<String, byte[]> getFields(final InputStream is) throws InvalidBEncodedStringException, IOException {
        final Map<String, byte[]> map = new HashMap<>();
        byte[] value, key;
        int c;
        var pbis = new PushbackInputStream(is);
     ...
     */
    public static String byteArrayToHexStr(final byte[] bytes) {
        final StringBuilder s = new StringBuilder();
        int parteAlta, parteBaixa;
        for (final byte aByte : bytes) {
            parteAlta = ((aByte >> 4) & 0xf) << 4;
            parteBaixa = aByte & 0xf;
            if (parteAlta == 0)
                s.append('0');
    ...
    public static String hexStrToStr(final String hexStr) {
        final byte[] b = hexStrToByteArray(hexStr);
        return new String(b);
    }
     ...
     */
    public static String byteArrayToStr(final byte[] b) {
